```

---

## 📄 MOC File (Standalone)

If you want to create the MOC separately, here's the complete content:

**File:** `00-Diffusion-LLM-MOC.md`
```markdown
# 🌐 Diffusion-LLM Knowledge Graph

> [!summary] Overview
> A curated collection of **47** diffusion language model papers, organized by theoretical foundations, architecture, acceleration methods, alignment techniques, and advanced capabilities.
> 
> **Last Updated:** 2025-01-15
> **Designed for:** Obsidian with bidirectional linking and Mermaid visualizations

## 📊 Statistics
| Category | Papers |
|----------|--------|
| Theoretical Basis | 6 |
| Foundation Model | 3 |
| Multimodal Understanding | 3 |
| Unified Multimodal Model | 1 |
| Speech ASR | 1 |
| Fast Sampling KV Cache | 7 |
| Advanced Sampling Method | 5 |
| Reinforcement Learning | 7 |
| Long Context | 1 |
| Variable Length | 4 |
| Others | 2 |

**Total:** 47 papers

## 🗺️ Table of Contents

## 📚 Theoretical Basis
- [[Deep Unsupervised Learning using Nonequilibrium Thermodynamics]]
- [[Structured Denoising Diffusion Models in Discrete State-Spaces]]
- [[Discrete Diffusion Modeling by Estimating the Ratios of the Data Distribution]]
- [[Your Absorbing Discrete Diffusion Secretly Models the Conditional Distributions of Clean Data]]
- [[Simplified and Generalized Masked Diffusion for Discrete Data]]
- [[Simple and Effective Masked Diffusion Language Models]]

## 📚 Foundation Model
- [[LLaDA: Large Language Diffusion Models]]
- [[Dream 7B]]
- [[Seed Diffusion: A Large-Scale Diffusion Language Model with High-Speed Inference]]

## 📚 Multimodal Understanding
- [[LLaDA-V: Large Language Diffusion Models with Visual Instruction Tuning]]
- [[LaViDa: A Large Diffusion Language Model for Multimodal Understanding]]
- [[Dimple: Discrete Diffusion Multimodal Large Language Model with Parallel Decoding]]

## 📚 Unified Multimodal Model
- [[MMaDA: Multimodal Large Diffusion Language Models]]

## 📚 Speech ASR
- [[Whisfusion: Parallel ASR Decoding via a Diffusion Transformer]]

## 📚 Fast Sampling KV Cache
- [[dKV-Cache: The Cache for Diffusion Language Models]]
- [[dLLM-Cache: Accelerating Diffusion Large Language Models with Adaptive Caching]]
- [[Accelerating Diffusion Language Model Inference via Efficient KV Caching and Guided Diffusion]]
- [[Fast-dLLM: Training-free Acceleration of Diffusion LLM by Enabling KV Cache and Parallel Decoding]]
- [[Esoteric Language Models]]
- [[Sparse-dLLM: Accelerating Diffusion LLMs with Dynamic Cache Eviction]]
- [[DPad: Efficient Diffusion Language Models with Suffix Dropout]]

## 📚 Advanced Sampling Method
- [[Variational Autoencoding Discrete Diffusion with Enhanced Dimensional Correlations Modeling]]
- [[Accelerated Sampling from Masked Diffusion Models via Entropy Bounded Unmasking]]
- [[Accelerating Diffusion LLMs via Adaptive Parallel Decoding]]
- [[Accelerating Diffusion Large Language Models with SlowFast Sampling: The Three Golden Principles]]
- [[Wide-In, Narrow-Out: Revokable Decoding for Efficient and Effective DLLMs]]

## 📚 Reinforcement Learning
- [[d1: Scaling Reasoning in Diffusion Large Language Models via Reinforcement Learning]]
- [[Reinforcing the Diffusion Chain of Lateral Thought with Diffusion Language Models]]
- [[LLaDA 1.5: Variance-Reduced Preference Optimization for Large Language Diffusion Models]]
- [[DiffuCoder: Understanding and Improving Masked Diffusion Models for Code Generation]]
- [[MDPO: Overcoming the Training-Inference Divide of Masked Diffusion Language Models]]
- [[Revolutionizing Reinforcement Learning Framework for Diffusion Large Language Models]]
- [[Inpainting-Guided Policy Optimization for Diffusion Large Language Models]]

## 📚 Long Context
- [[LongLLaDA: Unlocking Long Context Capabilities in Diffusion LLMs]]

## 📚 Variable Length
- [[Edit Flows: Flow Matching with Edit Operations]]
- [[DreamOn: Diffusion Language Models For Code Infilling Beyond Fixed-Size Canvas]]
- [[Beyond Fixed: Variable-Length Denoising for Diffusion Large Language Models]]
- [[Any-Order Flexible Length Masked Diffusion]]

## 📚 Others
- [[Time Is a Feature: Exploiting Temporal Dynamics in Diffusion Language Models]]
- [[Thinking Inside the Mask: In-Place Prompting in Diffusion LLMs]]

## 🔍 Quick Navigation
- [[#Theoretical Basis|Theoretical Basis]] - Foundation papers on diffusion theory
- [[#Foundation Model|Foundation Model]] - Large-scale diffusion LLMs
- [[#Multimodal Understanding|Multimodal Understanding]] - Vision-language diffusion models
- [[#Fast Sampling|Fast Sampling]] - KV caching and acceleration
- [[#Reinforcement Learning|Reinforcement Learning]] - RL alignment and reasoning
- [[#Long Context|Long Context]] - Extended context handling
- [[#Variable Length|Variable Length]] - Flexible sequence generation

## 📈 Graph View Tips
1. Enable **Graph View** in Obsidian to see connections
2. Use **Search** (Ctrl/Cmd+O) to find papers by keyword
3. Click any `[[wikilink]]` to navigate between related papers
4. Use **Backlinks** panel to see what references each paper

---
#diffusion-llm #knowledge-graph #obsidian #research
```

---

## 🚀 How to Run

### Step 1: Save the Script
```bash
# Save as generate_diffusion_kg.py
chmod +x generate_diffusion_kg.py
```

### Step 2: Run the Script
```bash
python3 generate_diffusion_kg.py
```

### Step 3: Open in Obsidian
```bash
# The script creates: Diffusion-LLM-Knowledge-Graph/
# Open this folder as an Obsidian vault
```

### Step 4: Enable Mermaid in Obsidian
1. Go to **Settings** → **Markdown**
2. Enable **Mermaid** diagrams
3. Install recommended plugins:
   - **Dataview** (for dynamic queries)
   - **Templater** (for templates)
   - **Advanced Tables** (for better tables)

---

## 📊 Output Structure
```
Diffusion-LLM-Knowledge-Graph/
├── 00-Diffusion-LLM-MOC.md
├── Theoretical-Basis/
│   ├── Theoretical-Basis-Index.md
│   └── Deep-Unsupervised-Learning-using-Nonequilibrium-Thermodynamics/
│       └── Deep-Unsupervised-Learning-using-Nonequilibrium-Thermodynamics.md
├── Foundation-Model/
│   ├── Foundation-Model-Index.md
│   └── LLaDA-Large-Language-Diffusion-Models/
│       └── LLaDA-Large-Language-Diffusion-Models.md
├── Fast-Sampling-KV-Cache/
│   ├── Fast-Sampling-KV-Cache-Index.md
│   └── dKV-Cache-The-Cache-for-Diffusion-Language-Models/
│       └── dKV-Cache-The-Cache-for-Diffusion-Language-Models.md
└── ... (all 11 categories)
```

---

## 🎯 Expected Output
| Metric | Count |
|--------|-------|
| Total Papers | 47 |
| Categories | 11 |
| Category Index Files | 11 |
| Paper Notes | 47 |
| MOC File | 1 |
| **Total Files** | **60** |

---

Let me know if you'd like me to:
1. **Add Dataview queries** for dynamic paper tables
2. **Create a graph visualization** export (Graphviz/Neo4j)
3. **Add paper summary templates** with auto-fetch from arXiv
4. **Generate batch updates** when new papers are added