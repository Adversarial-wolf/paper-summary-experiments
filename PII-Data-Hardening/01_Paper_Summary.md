Here is a complete, structured package for your `PII Data Hardening` folder. It includes a summary with Mermaid architecture diagrams, followed by a fully reproducible Jupyter Notebook that implements the experimental methodology described in Sections 4 & 5 of the paper.
...
PII Data Hardening/
├── 01_Paper_Summary.md
└── 02_Experimentation_Notebook.ipynb
```
...
# 🛡️ Hardening x402: PII-Safe Agentic Payments via Pre-Execution Metadata Filtering
**Author:** Vladimir Stantchev (April 2026)  
**Core Problem:** The `x402` micropayment protocol embeds plaintext metadata (`resource_url`, `description`, `reason`) into HTTP requests before on-chain settlement. This exposes PII to payment servers and facilitator APIs without DPAs, lacks spending circuit-breakers, and allows signed-token replay attacks.

## 🔑 Key Contributions
1. **`HardenedX402Client`**: A drop-in Python middleware that intercepts x402 requests pre-execution and applies 4 sequential controls.
2. **Synthetic Corpus**: 2,000 labeled metadata triples across 7 use-case categories for reproducible PII filter evaluation.
3. **42-Config Parameter Sweep**: Benchmarks regex vs. NLP detection, entity subsets, and confidence thresholds.
4. **Recommended Config**: `mode=nlp`, `all_entities`, `min_score=0.4` → `µF1=0.894`, `Precision=0.972`, `p99 Latency=5.73ms`.

## 🏗️ System Architecture & Flow
```mermaid
flowchart TD
    A[AI Agent] -->|GET /resource| B[x402 Server]
    B -->|HTTP 402 + PaymentSpec| C[HardenedX402Client]
    
    subgraph Pre-Execution Interception
        C --> D{PIIFilter\nPresidio Scan & Redact}
        D -->|PII_FOUND| E[Redact & Emit PII_REDACTED]
        D -->|CLEAN| F{PolicyEngine\nSpend Limits}
        E --> F
        F -->|OK| G{ReplayGuard\nHMAC Dedup}
        F -->|OVER_LIMIT| H[Block & Emit POLICY_BLOCKED]
        G -->|NEW| I[AuditLog\nJSON-L + HMAC Chain]
        G -->|DUPLICATE| J[Block & Emit REPLAY_BLOCKED]
        I --> K[Sign EIP-712 Token]
    end
    
    K -->|X-Payment Header| L[Base L2 Facilitator API]
    L -->|On-chain Receipt| B
    B -->|200 OK + Resource| A
```
...
```mermaid
graph TD
    Sweep[42-Config Parameter Sweep] --> M1[Mode: Regex vs NLP]
    Sweep --> M2[Entity Subsets: 1-6 types]
    Sweep --> M3[Confidence: min_score ∈ {0.3, 0.4, 0.5, 0.6, 0.7}]
    
    M1 & M2 & M3 --> Eval[Run against 2,000 synthetic triples]
    Eval --> R1[Regex: Perfect on structural PII, 0.000 PERSON recall]
    Eval --> R2[NLP: 0.551 PERSON recall, p99=5.73ms]
    
    R1 & R2 --> Opt[Optimal: NLP + all entities + min_score=0.4]
    Opt --> Metrics[µF1=0.894 | P=0.972 | R=0.827]
    Metrics --> Conclusion[<50ms budget. NLP latency is insurance, not overhead.]
```

## 📊 Key Takeaways
- **Fail-safe over fail-open:** Unhandled PII filter exceptions block payments. False blocks are cheaper than PII leaks.
- **PERSON Recall Ceiling:** NLP struggles with URL slugs (`/alice-martin/`). Recall caps at `0.551` without domain-adapted NER.
- **Latency is not the bottleneck:** NLP adds `5.73ms` p99, well within the `50ms` agent-speed budget. The tradeoff for `+20% micro-recall` is architecturally sound.
- **Governance > Code:** The middleware provides a technical harness; deployment scope remains a legal/policy decision.

---

### 📓 `02_Experimentation_Notebook.ipynb`

Below is the exact content for your Jupyter Notebook. Copy each `%%markdown` and code cell directly into your `.ipynb` file.
...
### 🛠️ How to Use
1. Create a folder named `PII Data Hardening`.
2. Save the markdown content as `01_Paper_Summary.md`.
3. Save the JSON block exactly as `02_Experimentation_Notebook.ipynb` (or paste the markdown/code cells directly into a new Jupyter Notebook).
4. Run the first code cell to install dependencies (`presidio-analyzer`, `spacy`, etc.).
5. Execute cells sequentially. The notebook will generate the synthetic corpus, run the 42-configuration sweep, and output a validation table matching **Table 3** from the paper.
